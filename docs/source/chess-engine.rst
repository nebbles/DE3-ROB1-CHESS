...
The chess engine is an interface to any chess AI we choose. Currently we are using
`sunfish by thomasahle`_ as our chess AI of choice. The chess engine spins up an instance of the
...
The CE (chess engine) is written by us, it can provide us with all the
chess functionality we need.
...
We take the BWE matrix provided by the Perception module. This Black-White-Empty
...
We compare this matrix with an internally stored matrix in the CE. This means the CE can
understand:

* where the piece moved from
* where it moved to

and construct a chess command from it. This BWE matrix si of course checked for logical inconsistencies by measuring the change in number of black, white or empty squares in a single turn. Now that we have a potential chess move, we add it to the command queue, a shared resource that both the CE and the chess AI (sunfish) have access to.
