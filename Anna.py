...
class MotionPlanner:
    def __init__(self, arm_object, visual=False, manual_calibration=False, debug=False):
        ...
        rest_y_vector = [-(i * 1) for i in self.y_unit_vector]
    ...
    def apply_trapezoid_vel_profile(self, path):  # todo docstring and finish the function
        ...
        for i in range(1, len(speed_values)):
            ...
            if smooth_path_idx > len(smooth_path):
                smooth_path_idx = len(smooth_path)
        ...
        # trajectory[-1, -1] = 0
        # final position and speed missing
        # for i in range(3):
        #     new_marker = np.hstack((smooth_path[-1], speed_values[-1]))  # main technique
        #     trajectory = np.vstack((trajectory, new_marker))

        print("path -1: ", path[-1])
        print(smooth_path[-10:,:])
        print("traj last few: \n", trajectory[-4:, :])
...
def anna(self, chess_move):
    ...
    if len(chess_move) == 1:  # NO PIECE DIED
        ...
        moves = np.array([[[current_position, self.rest], [current_position, move_to_hover], [current_position, move_to]],
                          [[current_position, current_position], [current_position, dead_zone_hover]],
...
if __name__ == '__main__':
    ...
    # path_1 = np.array([[0, 0, 0], [0, 1, 0], [1, 1, 0]])  # right angle turn - nope
    # path_2 = np.array([[1, 2, 0], [2, 0.5, 0], [4, 4, 0]])  # tight angle turn - nope
    # path_3 = np.array([[0, 0, 0], [0, 0.5, 0]])  # straight line example
    # path_4 = np.array([[0.5, -0.25, 0.2], [0.5, 0.25, 0.2]])  # move +y


    #motion_plan = planner.apply_trapezoid_vel_profile(path_3)



    motion_plans = anna(planner, [("r", "a1a6")])
    # motion_plans = anna(planner, [("r", "b4"), ("r", "a1b4")])
    fig = plt.figure()
    ax3d = fig.add_subplot(111, projection='3d')
    for motion_plan in motion_plans:
        ax3d.plot(motion_plan[:, 0], motion_plan[:, 1], motion_plan[:, 2], 'r*')
    ...
    # getting board points
    board_x = [coord[0] for coord in planner.board]
    board_y = [coord[1] for coord in planner.board]
    board_z = [coord[2] for coord in planner.board]
    # add the first point to the end of the list to create polygon
    board_x.append(planner.board[0][0])
    board_y.append(planner.board[0][1])
    board_z.append(planner.board[0][2])
