...
void subscribe_target_motion() {
  ...
  ros::Subscriber sub_move = node_handle.subscribe("franka_move_to", 1, motionCallback);  // 1: buffer size of message queue
  ros::Subscriber sub_grip = node_handle.subscribe("franka_gripper_control", 1, gripperCallback);
